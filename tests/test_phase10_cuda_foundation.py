...
def test_phase_ten_public_cuda_transfer_surface_is_exposed() -> None:
    assert hasattr(fastpauli, "DevicePauliSum")
    assert hasattr(fastpauli, "DeviceCommutationMatrix")
    assert hasattr(fastpauli.PauliSum, "to_device")
    assert hasattr(fastpauli, "cuda_available")
    assert hasattr(fastpauli, "cuda_devices")
...
def test_cuda_status_distinguishes_build_time_absence() -> None:
    ...
    assert "FASTPAULI_CUDA_HOST_COMPILER" in build_info["compiler_build_config"]
    assert "FASTPAULI_CUDA_HOST_COMPILER_SOURCE" in build_info["compiler_build_config"]
...
def test_cmake_cuda_build_records_host_compiler_without_forcing_selection() -> None:
    ...
    assert "FASTPAULI_CUDA_HOST_COMPILER_METADATA" in cmake


def test_cmake_cuda_build_pins_cudatoolkit_root_from_selected_compiler() -> None:
    cmake = (ROOT / "CMakeLists.txt").read_text(encoding="utf-8")

    assert "get_filename_component(FASTPAULI_CUDAToolkit_ROOT_FROM_CMAKE_CUDA_COMPILER" in cmake
    assert "set(CUDAToolkit_ROOT \"${FASTPAULI_CUDAToolkit_ROOT_FROM_CMAKE_CUDA_COMPILER}\" CACHE PATH \"Resolved CUDA toolkit root\" FORCE)" in cmake
    assert "set(CUDAToolkit_ROOT \"${FASTPAULI_CUDAToolkit_ROOT_FROM_CUDACXX}\" CACHE PATH \"Resolved CUDA toolkit root\" FORCE)" in cmake
...
def test_to_device_reports_clear_cpu_only_error_or_round_trips() -> None:
    ...
    assert isinstance(device_op, fastpauli.DevicePauliSum)

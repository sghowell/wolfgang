...
`FASTPAULI_ENABLE_INTERNAL_BINDINGS=OFF` is the default for every source and wheel
build. Repository developers explicitly opt in with
`-Ccmake.define.FASTPAULI_ENABLE_INTERNAL_BINDINGS=ON` when running validation
...
Official release wheels set `FASTPAULI_ENABLE_INTERNAL_BINDINGS=OFF` explicitly in

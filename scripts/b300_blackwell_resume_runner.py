...
export FASTPAULI_CUDA_ARCHITECTURES='100-real;120'
...
run_step install_editable sh -c 'python -m pip install -e .[test] --config-settings=cmake.define.FASTPAULI_ENABLE_INTERNAL_BINDINGS=ON --config-settings=cmake.define.FASTPAULI_ENABLE_CUDA=ON --config-settings=cmake.define.FASTPAULI_ENABLE_HIP=OFF --config-settings=cmake.define.FASTPAULI_ENABLE_METAL=OFF --config-settings=cmake.define.FASTPAULI_CUDA_ARCHITECTURES=100-real\;120 --config-settings=cmake.define.FASTPAULI_ENABLE_NATIVE=OFF > "$evidence/private/logs/cuda-install.log" 2>&1'

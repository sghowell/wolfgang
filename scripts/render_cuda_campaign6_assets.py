#!/usr/bin/env python3
"""Render Campaign 6 H100 CUDA consumer-count summary and SVG assets."""
...
RAW_FILES = {
    "consumer_scaling": "experiment_campaign6_consumers.json",
    "device_output_reference": "experiment_campaign5_device_output_reference.json",
    "default_cross": "experiment_cuda_kernels_default.json",
    "competitive": "competitive_baselines_final.json",
}

OPTIONAL_RAW_FILES: dict[str, str] = {}

PLOTS = {
    "consumer_pipeline": "cuda_h100_campaign6_consumer_pipeline.svg",
    "cupy_consumer": "cuda_h100_campaign6_cupy_consumer.svg",
    "landscape": "cuda_h100_campaign6_performance_landscape.svg",
    "status": "cuda_h100_campaign6_evidence_status.svg",
}
...
SERIES_COLORS = {
    ...
    "CUDA compact count": "#0891b2",
    "CUDA row count": "#0e7490",
    "CUDA column count": "#0369a1",
    "CuPy reduction": "#e11d48",
    "CUDA dense to_host": "#65a30d",
...
def load_raw(data_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    if missing:
        raise FileNotFoundError("missing Campaign 6 raw artifact(s): " + ", ".join(missing))
...
def competitor_landscape_row(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if case["name"] == "cupy_commutation_consumer":
        add_speedup_point(
            points,
            "CUDA compact count",
            baseline,
            results.get("fastpauli_cuda_compact_count_seconds"),
        )
        add_speedup_point(
            points,
            "CUDA dense to_host",
            baseline,
            results.get("competitor_dense_to_host_seconds"),
        )
    ...
    elif case["name"] == "cupy_commutation_consumer":
        name = "CuPy consumer"
    label_bits = []
    if "num_terms" in dataset:
...
def build_performance_landscape(raw: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    consumer_rows = [
        consumer_landscape_row(case)
        for case in raw["consumer_scaling"]["cases"]
    ]
    competitor_rows = [competitor_landscape_row(case) for case in raw["competitive"]["cases"]]
    return default_rows + consumer_rows + competitor_rows


def consumer_landscape_row(case: dict[str, Any]) -> dict[str, Any]:
    results = case["results"]
    baseline = float(results["cpu_scalar_seconds"])
    points: list[dict[str, Any]] = [
        {
    ...
    add_speedup_point(points, "CUDA transfer-inclusive", baseline, results.get("cuda_transfer_inclusive_seconds"))
    add_speedup_point(points, "CUDA device-output reuse", baseline, results.get("cuda_device_output_reuse_seconds"))
    add_speedup_point(points, "CUDA compact count", baseline, results.get("cuda_device_output_consumer_total_seconds"))
    add_speedup_point(points, "CUDA row count", baseline, results.get("cuda_device_output_consumer_axis0_seconds"))
    add_speedup_point(points, "CUDA column count", baseline, results.get("cuda_device_output_consumer_axis1_seconds"))
    add_speedup_point(points, "CUDA dense to_host", baseline, results.get("cuda_device_output_dense_to_host_seconds"))
    add_speedup_point(points, "CuPy reduction", baseline, results.get("cupy_sum_total_seconds"))
    return {
        "category": "Campaign 6 consumer scaling",
        "label": f"commutation consumer {scale_label(str(case['scale']))}",
        "operation": "pairwise_commutation_consumer",
        "scale": case["scale"],
        "points": points,
    }


def consumer_pipeline_rows(report: dict[str, Any]) -> list[dict[str, Any]]:
    rows = []
    for case in report["cases"]:
        results = case["results"]
        rows.append(
            {
                ...
                "cpu_scalar_seconds": results["cpu_scalar_seconds"],
                "cpu_optimized_seconds": results.get("cpu_optimized_seconds"),
                "fill_reuse_seconds": results["cuda_device_output_reuse_seconds"],
                "compact_total_seconds": results["cuda_device_output_consumer_total_seconds"],
                "compact_axis0_seconds": results["cuda_device_output_consumer_axis0_seconds"],
                "compact_axis1_seconds": results["cuda_device_output_consumer_axis1_seconds"],
                "compact_to_host_bytes": results["cuda_device_output_consumer_to_host_bytes"],
                "dense_to_host_seconds": results["cuda_device_output_dense_to_host_seconds"],
                "cupy_total_seconds": results.get("cupy_sum_total_seconds"),
                "cupy_axis0_seconds": results.get("cupy_sum_axis0_seconds"),
                "cupy_axis1_seconds": results.get("cupy_sum_axis1_seconds"),
                "cupy_dense_to_host_seconds": results.get("cupy_dense_to_host_seconds"),
            }
        )
    return rows
...
def render_device_output_boundaries(rows: list[dict[str, Any]], output: Path) -> None:
    series = [
        ("CPU optimized", "cpu_optimized_seconds", COLORS["purple"], -15),
        ("device fill reuse", "fill_reuse_seconds", COLORS["green"], -6),
        ("compact total count", "compact_total_seconds", "#0891b2", 2),
        ("compact row count", "compact_axis0_seconds", "#0e7490", 10),
        ("compact column count", "compact_axis1_seconds", "#0369a1", 18),
        ("dense to_host", "dense_to_host_seconds", "#65a30d", 26),
    ]
    values = [
        float(row[key])
        for row in rows
    ...
    height = legend_top + 112
    x_for, min_log, max_log = log_scale_mapper(values, left, right)
    lines = svg_start(
        width,
        height,
        "Campaign 6 Consumer Pipeline",
        "Pairwise commutation consumer latencies on H100; lower latency is farther left on the log axis.",
    )
    ...
    for index, (label, _, color, _) in enumerate(series):
        x = 32 + (index % 3) * 390
        y = legend_top + (index // 3) * 28
        lines.append(circle(x, y - 4, 5.3, color))
        lines.append(text(x + 14, y, label, size=11, color=COLORS["muted"]))
    lines.append(text(32, height - 24, "Source: Campaign 6 checked raw JSON. Compact counts return scalar or per-axis counts without materializing the dense matrix on host.", size=11, color=COLORS["muted"]))
    lines.append("</svg>")
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def render_host_decomposition(rows: list[dict[str, Any]], output: Path) -> None:
    ...
    top = 118
    row_h = 46
    legend_top = top + len(rows) * row_h + 42
    height = legend_top + 72
    series = [
        ("FastPauli compact total", "compact_total_seconds", "#0891b2", -8),
        ("CuPy dense total", "cupy_total_seconds", "#e11d48", 0),
        ("FastPauli dense to_host", "dense_to_host_seconds", "#65a30d", 8),
        ("CuPy dense to_host", "cupy_dense_to_host_seconds", COLORS["amber"], 16),
    ]
    values = [
        float(row[key])
        for row in rows
    ...
    lines = svg_start(
        width,
        height,
        "Campaign 6 CuPy Consumer Comparison",
        "Dense CUDA Array Interface reductions versus FastPauli compact device-side counts; lower is better.",
    )
    for power in range(int(min_log), int(max_log) + 1):
        tick = 10**power
        x = x_for(tick)
    ...
    for index, row in enumerate(rows):
        ...
        if index % 2 == 0:
            lines.append(rect(24, y - 20, width - 48, row_h - 2, "#f9fafb", rx=0))
        lines.append(text(32, y + 3, scale_label(str(row["scale"])), size=12))
        lines.append(text(176, y + 3, f"{int(row['entries']):,} entries", size=10, color=COLORS["muted"]))
        for _, key, color, offset in series:
            value = row.get(key)
            if value is not None:
                lines.append(circle(x_for(float(value)), y + offset, 5.2, color))
        fast = float(row["compact_total_seconds"])
        cupy = row.get("cupy_total_seconds")
        if cupy:
            lines.append(text(1120, y + 3, f"CuPy/FastPauli {ratio(float(cupy) / fast)}", size=10, color=COLORS["muted"], anchor="end"))
    for index, (label, _, color, _) in enumerate(series):
        x = 32 + index * 260
        y = legend_top
        lines.append(circle(x, y - 4, 5.2, color))
        lines.append(text(x + 14, y, label, size=11, color=COLORS["muted"]))
    lines.append("</svg>")
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def render_performance_landscape(rows: list[dict[str, Any]], output: Path) -> None:
    ...
    lines = svg_start(
        ...
        "Campaign 6 broad comparison: CPU selectors, CUDA kernels, compact consumers, CuPy, cuQuantum, Qiskit, and OpenFermion.",
    )
    ...
    series_offsets = {
        ...
        "CUDA compact count": 14,
        "CUDA row count": 18,
        "CUDA column count": 22,
        "CuPy reduction": -18,
        "CUDA dense to_host": 26,
    ...
    lines.append(text(32, height - 24, "Source: Campaign 6 checked raw JSON. Consumer rows include large dense commutation matrices plus compact count reducers.", size=11, color=COLORS["muted"]))
    lines.append("</svg>")
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def collect_evidence_status(data_dir: Path) -> list[dict[str, str]]:
    ...
    privileged_ncu_ok = log_contains(
        profiler / "ncu_campaign6_consumers_details.csv",
        "Metric Name",
    ) and log_contains(profiler / "ncu_campaign6_count_kernels_details.csv", "Metric Name")
    unprivileged_ncu_permission = log_contains(
        profiler / "ncu_campaign6_consumers.log",
        "ERR_NVGPUCTRPERM",
    )
    return [
        {
        ...
        },
        {
            ...
            "status": "passed" if log_contains(metadata / "experiment-phase11-cuda.log", "23 passed") else "missing_or_failed",
        },
        {
        ...
        {
            "label": "Campaign 6 consumer benchmark",
            "status": "passed" if (data_dir / "raw" / RAW_FILES["consumer_scaling"]).exists() else "missing_or_failed",
        },
        {
        ...
        },
        {
            ...
            "status": "passed" if any(path.exists() for path in profiler.glob("nsys_campaign6_consumers_stats*")) else "missing_or_failed",
        },
        {
...
def build_summary(raw: dict[str, dict[str, Any]], data_dir: Path) -> dict[str, Any]:
    ...
    scaling = raw["consumer_scaling"]
    default_cross = raw["default_cross"]
    boundary_rows = consumer_pipeline_rows(scaling)
    statuses = collect_evidence_status(data_dir)
    return {
        "campaign": "h100_campaign6",
        "date": "2026-04-29",
        ...
        "decisions": [
            {
                "experiment": "consumer_count_api",
                "status": "implemented",
                "reason": "DeviceCommutationMatrix.count_commuting(axis=None|0|1) keeps common reductions on device and returns compact counts.",
            },
            {
                "experiment": "stream_or_async_api",
                "status": "deferred",
                "reason": "Public CUDA stream/async lifetime contracts remain deferred until an explicit API design is accepted.",
            },
            {
                "experiment": "bit_packed_commutation_output",
                "status": "deferred",
                "reason": "Campaign 6 validates compact consumer reductions first; dense uint8 remains the public GPU-resident matrix representation.",
            },
            {
                "experiment": "cupy_consumer_baseline",
                "status": "captured",
                "reason": "CuPy reduction timings on DeviceCommutationMatrix CUDA Array Interface are tracked against FastPauli compact reducers.",
            },
            {
                "experiment": "readme_plot_policy",
                "status": "refreshed_broad_landscape",
                "reason": "Campaign 6 updates the broad CPU/CUDA/external plot with compact consumer rows and CuPy comparisons.",
            },
        ],
        "consumer_pipeline": boundary_rows,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("docs/benchmarks/data/cuda_deep_optimization_h100_campaign6_2026-04-29"),
    )
...
def main() -> int:
    ...
    boundary_rows = summary["consumer_pipeline"]
    render_device_output_boundaries(boundary_rows, args.plot_dir / PLOTS["consumer_pipeline"])
    render_host_decomposition(boundary_rows, args.plot_dir / PLOTS["cupy_consumer"])

...
DevicePauliSum DevicePauliSum::matmul(
  ...
  const int blocks = out_terms == 0 ? 0 : checked_launch_blocks(out_terms, "HIP matmul");
  ...
  const std::size_t packed_words =
      detail::checked_product(out_terms, impl_->words, "HIP matmul packed words");
  hip_allocate(out->x, packed_words, "HIP matmul x words");
  hip_allocate(out->z, packed_words, "HIP matmul z words");
  hip_allocate(out->coeffs, out_terms, "HIP matmul coefficients");
  matmul_product_kernel<<<blocks, kHipThreadsPerBlock>>>(
      impl_->x,
      impl_->z,
      impl_->coeffs,
      rhs.impl_->x,
      ...
      out->z,
      out->coeffs);
  check_hip(hipGetLastError(), "launch HIP matmul");

  DevicePauliSum product(std::move(out));
  if (simplify_output) {
    return product.simplify();
  }
  check_hip(hipDeviceSynchronize(), "synchronize after HIP matmul");
  return product;
}
